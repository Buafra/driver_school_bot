...
# - Notification to driver when a REAL trip is added# driver_school_bot.py
# DriverSchoolBot — Multi-driver Shared Ledger with Driver & Admin Menus
#
# Features:
# - Admins: you (Faisal) + Abdulla
...
def weekly_range_now(data: Dict[str, Any]) -> Tuple[Optional[datetime], datetime]:
    """
    Get (start_of_week, end_of_week) for weekly report,
    respecting weekly_start_date if set.

    Logic:
    - If weekly_start_date is set and not in the future, we anchor all weeks to it
      in 7-day blocks.
    - If weekly_start_date is in the future → return (None, now).
    - If weekly_start_date is not set, we use this calendar week Monday–Friday.
    - End of the week is capped to today (no future days).
    """
    ...
    if floor_date:
        # Anchor weeks to the configured start date in 7-day blocks
        days_diff = (today - floor_date).days
        week_index = days_diff // 7
        week_start = floor_date + timedelta(days=7 * week_index)
    else:
        # Default: current calendar week starting Monday
        week_start = today - timedelta(days=today.weekday())

    # Week ends on Friday (Mon–Fri window)
    ...
    end_dt = datetime(
        week_end_date.year, week_end_date.month, week_end_date.day, 23, 59, 59, tzinfo=DUBAI_TZ
    )
