...
# DriverSchoolBot — Shared Ledger for Driver Extra Trips (Multi-Driver, Notifications)
#
# Features:
# - Admins: you (Faisal) + Abdulla
# - Multiple drivers, each with their own trips
# - Primary driver for simple /trip and quick text
# - Weekly / monthly / yearly reports
#   • Calculations start from /setweekstart YYYY-MM-DD
#   • Old history kept, but ignored in totals
...
# - Test mode, export CSV, filter, destination search
#
# Requirements:
#   python-telegram-bot==21.4
#
# Environment:
#   BOT_TOKEN must be set in Render
...
def clamp_start_by_weekstart(data: Dict[str, Any], start_date: date) -> Optional[date]:
    ...
    Returns None if the whole period is before start.
    """
...
def compute_weekly_totals(
    ...
    # apply weekly_start_date clamp at trip level as extra safety
    floor_str = data.get("weekly_start_date")
    floor_date = parse_date_str(floor_str) if floor_str else None
    ...
    # school base (global) for this week: full week base adjusted by no-school days
...
async def notify_driver_for_trip(
    ...
    if not driver_id:
        return
    if trip.get("is_test"):
        # don't spam driver for test trips
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if admin:
        msg += (
            ...
            "• `/noschool [today|tomorrow|YYYY-MM-DD]` – mark no school + notify drivers\n"
            ...
            "• `/setprimarydriver <id>` – mark primary driver\n"
            "• `/paid [amount]` – send payment notification with amount\n"
    ...
    if driver_flag:
        msg += (
            "\nDriver commands:\n"
            "• `/driverview` – quick view of this week trips (extra only)\n"
            "• `/driverview_report` – detailed weekly report (extra only)\n"
        )

    if update.message:
        await update.message.reply_text(msg, parse_mode="Markdown")

    # Optionally show menu for admins
...
async def add_trip_internal(
    ...
    d = get_driver_by_id(data, driver_id)
    driver_name = d["name"] if d and d.get("name") else str(driver_id)
...
# ----- Quick trip from plain text -----

async def quick_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Handle non-command text:
    - If admin and text starts with a number → quick trip: "<amount> <destination>"
    - Otherwise ignore (or could be from driver).
    """
    ...
    if not is_admin(user.id):
        # we can ignore or later add driver convenience commands
        return
...
# ----- Admin: weekly / month / year reports -----
...
async def month_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # clamp by weekly_start_date
    clamped = clamp_start_by_weekstart(data, start_date)
    if clamped is None or clamped > end_date:
        await update.message.reply_text(
            f"No data for {year}-{month:02d} (before driver start).",
            parse_mode="Markdown",
        )
        return
    start_date = clamped
    ...
    # filter by date & weekly_start_date
    filtered = []
...
async def year_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if clamped is None or clamped > end_date:
        await update.message.reply_text(
            f"No data for {year} (before driver start).",
            parse_mode="Markdown",
        )
        return
    start_date = clamped
    ...
    if not context.args:
        ...
        if arg in ("today", "td"):
            ...
            try:
                ...
                await update.message.reply_text("Use: `/noschool [today|tomorrow|YYYY-MM-DD]`", parse_mode="Markdown")
                return
...
async def export_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write("id,driver_id,date,amount,destination,user_id,user_name,is_test\n")
        for t in sorted(trips, key=lambda x: x["id"]):
            dest = t["destination"].replace('"', '""')
            uname = (t.get("user_name") or "").replace('"', '""')
            f.write(
                f"{t['id']},{t.get('driver_id','')},{t['date']},{t['amount']},"
...
async def driverview_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Called by driver account (Telegram ID must match driver id)
    """
    user = update.effective_user
...
async def driverview_report_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    For driver: /driverview_report
    For admin: /driverview_report <driver_id>
    """
    data = load_data()
    user = update.effective_user
    if not user:
...
async def menu_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text == BTN_ADD_TRIP:
        await update.message.reply_text(
            "Use `/trip <amount> <destination>` or send: `20 Dubai Mall`.",
            parse_mode="Markdown",
        )
    elif text == BTN_LIST:
    ...
    elif text == BTN_NOSCHOOL:
        await update.message.reply_text(
            "Use `/noschool today`, `/noschool tomorrow` or `/noschool YYYY-MM-DD`.",
            parse_mode="Markdown",
        )
    elif text == BTN_EXPORT:
        await export_cmd(update, context)
    elif text == BTN_CLEAR_TRIPS:
        await cleartrips_cmd(update, context)
    elif text == BTN_TEST_TOGGLE:
        data = load_data()
        ...
        # If it's not a known button, treat as quick text handler
        await quick_text_handler(update, context)
...
def main() -> None:
    ...
    # Text handler (menu buttons + quick trip)
    app.add_handler(
        MessageHandler(
