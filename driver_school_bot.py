...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Authorized users are also added as weekly-report subscribers and see the menu.
    """
    user = update.effective_user
    ...
    msg = (
        ...
        "• `/noschool` – mark no-school day\n"
        "• `/holiday` – holiday range\n"
        "• `/cleartrips` – delete all trips\n"
        "• `/test_on` / `/test_off` – toggle test mode\n\n"
        "🔔 Auto weekly report every *Friday 10:00 (Dubai)* for authorized chats."
    )

    if update.message:
        await update.message.reply_text(msg, parse_mode="Markdown")

    # Auto-open menu only for authorized users
    if is_auth and update.message:
...
async def menu_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Not a button → try quick-trip format: "<amount> <destination>"
