...
def clamp_start_by_weekstart(data: Dict[str, Any], start_date: date) -> Optional[date]:
    ...
    Returns None if the whole period is before it.
    """
...
async def notify_admins_for_trip(
    ...
    added_by = trip.get("user_id")
    driver_id = trip.get("driver_id")
    dt = parse_iso_datetime(trip["date"]).astimezone(DUBAI_TZ)
    d_str = dt.strftime("%Y-%m-%d %H:%M")

    driver_label = f"driver {driver_id}" if driver_id is not None else "no driver"
    ...
    text = (
        ...
        f"📍 {trip['destination']}\n"
        f"💰 {trip['amount']:.2f} AED\n"
        f"👤 Added by Telegram ID: {added_by}\n"
        f"🚗 For: {driver_label}"
    )

    from_ids = set(ALLOWED_USERS)
    for admin_id in from_ids:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if driver_flag:
        msg += (
            ...
            "• 🧾 My Weekly Report – detailed trips\n"
        )
...
async def add_trip_internal(
    ...
    # Notify other admins (e.g. send to Faisal when Abdulla adds a trip)
    await notify_admins_for_trip(context, trip)
